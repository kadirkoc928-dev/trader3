...
st.set_page_config(page_title="TradeScanner Pro - Alle Märkte + Wavetrend", page_icon="📊", layout="wide")
...
show_wavetrend = st.sidebar.checkbox("Wavetrend anzeigen", value=True)
...
st.sidebar.caption("⏱ ~10-20 Min für alle Aktien")
st.sidebar.caption("📊 Yahoo Finance (verzögert)")

# =============================================
# WAVETREND INDIKATOR
# =============================================
def calculate_wavetrend(df, ch1=10, ch2=21, ch3=34):
    """Wavetrend Indikator nach LazyBear"""
    try:
...
# =============================================
# TICKER-LISTEN (ALLE Aktien dieser Märkte)
# =============================================
...
def swing_score(df):
    ...
    try:
        ...
        if pd.notna(latest.get('WT1')) and pd.notna(latest.get('WT2')):
            ...
            if latest['WT1'] < -50: score += 3  # Überverkauft
...
def get_wavetrend_signal(wt1, wt2):
    """Wavetrend Signal-Interpretation"""
    if pd.isna(wt1) or pd.isna(wt2): return "N/A", "gray"
    if wt1 > wt2:
        if wt1 < -50: return "Bullish (überverkauft)", "#00ff88"
        elif wt1 > 50: return "Bullish (überkauft)", "#88ff00"
        else: return "Bullish", "#44ff00"
    else:
        if wt1 > 50: return "Bearish (überkauft)", "#ff4444"
        elif wt1 < -50: return "Bearish (überverkauft)", "#ff8800"
...
def scan_one(ticker):
    try:
        ...
        wt_signal, wt_color = get_wavetrend_signal(wt1_v, wt2_v)
        ...
        return {
            ...
            'Wavetrend': wt_signal, 'WT_Color': wt_color,
...
st.markdown("### ALLE Aktien dieser Märkte + Wavetrend-Indikator")
    ...
    if results:
        ...
        if not df_filtered.empty:
            for i, (_, row) in enumerate(df_filtered.iterrows()):
                ...
                cols = st.columns([0.5, 1.5, 0.7, 0.6, 0.6,
