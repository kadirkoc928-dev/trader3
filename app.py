...
def wt_signal(wt1, wt2):
    ...
    if wt1 > wt2:
        if wt1 < -50:
            return "Bullish OS", "#00ff88"
        else:
            return "Bullish", "#44ff00"
    else:
        if wt1 > 50:
            return "Bearish OB", "#ff4444"
        else:
...
def scan_one(ticker):
    try:
        ...
        if pd.isna(sma_v):
            ...
            sma_s = 'Above' if price > sma_v else 'Below'
        macd = latest.get('MACD')
        macd_s = latest.get('MACD_signal')
        if pd.isna(macd) or pd.isna(macd_s):
            macd_st = 'N/A'
        else:
            macd_st = 'Bullish' if macd > macd_s else 'Bearish'
...
st.success("✅ **" + str(len(all_tickers)) + " Ticker geladen**")
...
if st.button("🚀 ELITE SCAN STARTEN", type="primary", use_container_width=True):
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for t in all_tickers:
            futures[executor.submit(scan_one, t)] = t
        for future in as_completed(futures):
            ...
            if completed % 100 == 0 or completed == total:
                ...
                status_text.text("✅ " + str(completed) + "/" + str(total) + " | 🎯 " + str(len(results)) + " Elite-Treffer")
                time_text.text("⏱ " + str(int(elapsed)) + "s | ⏳ ~" + str(int(remaining)) + "s")
    ...
    if len(results) > 0:
        ...
        st.markdown("---")
        st.subheader("🏆 " + str(len(df_filtered)) + " Elite-Kandidaten (Score >=" + str(min_score) + ")")
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric("📊 Treffer", len(df_filtered))
        with c2:
            st.metric("⏱ Zeit", str(int(total_time)) + "s")
        with c3:
            best = df_filtered['Elite-Score'].max() if not df_filtered.empty else 0
            st.metric("🏆 Bester", str(int(best)) + "/100")
        ...
        if not df_filtered.empty:
            for i, (_, row) in enumerate(df_filtered.iterrows()):
                ...
                macd_color = "#00ff88" if row['MACD'] == 'Bullish' else "#ff4444"
                
                cols = st.columns([0.5, 1.0, 0.6, 0.5, 0.5, 0.5, 0.7, 0.7, 0.8, 0.6])
                cols[0].markdown("<span style='color:" + color + ";font-weight:bold;font-size:16px'>" + emoji + " " + str(score) + "</span>", unsafe_allow_html=True)
                cols[1].markdown("**" + str(row['Ticker']) + "**\n*" + str(row['Name'])[:25] + "*")
                cols[2].markdown("$" + str(row['Preis']))
                cols[3].markdown("RSI " + str(int(row['RSI'])))
                cols[4].markdown("ADX " + str(int(row['ADX'])))
                cols[5].markdown("RS " + str(int(row['RS-Rating'])))
                cols[6].markdown("<span style='color:" + str(row['WT_Color']) + ";font-size:11px'>" + str(row['Wavetrend']) + "</span>", unsafe_allow_html=True)
                cols[7].markdown("<span style='color:" + macd_color + "'>" + str(row['MACD']) + "</span>", unsafe_allow_html=True)
                cols[8].markdown("<span style='font-size:10px;color:#ffd700'>" + str(row['Badges']) + "</span>", unsafe_allow_html=True)
